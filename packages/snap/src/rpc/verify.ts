...
export function verifyLSAG(b64Signature: string): { isValid: boolean, content: { ring: string[], challenge: string, responses: string[], evmWitnesses?: string[] | undefined } | null } {
  try {
    ...
    return {
      ...
      content: {
        ...
        evmWitnesses: (sig.getEvmWitnesses() && sig.getEvmWitnesses().length > 0) ? sig.getEvmWitnesses().map((witness) => witness.toString()) : undefined
      },
    };
  } catch (e) {
...
// if the signature is well encoded, also returns its content
export function verifySAG(b64Signature: string): { isValid: boolean, content: { ring: string[], challenge: string, responses: string[] } | null } {
  try {
