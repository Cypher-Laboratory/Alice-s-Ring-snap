...
export const onRpcRequest: OnRpcRequestHandler = async ({
  ...
  switch (request.method) {
    ...
    case 'exportAccount': { // exportAccount -> called by the front end to export an account (requires user approval) (front end does access the mnemonic. user have to copy it from the snap dialog and paste it in a safe place)
    ...
    case 'SAG_Signature': { // SAG_Signature -> called by the front end to sign a message using the SAG scheme (requires user approval)
    ...
    case 'LSAG_Signature': { // LSAG_Signature -> called by the front end to sign a message using the LSAG scheme (requires user approval)
    ...
    case 'ExportKeyImages': { // ExportKeyImages -> called by the front end to export key images (requires user approval)
    ...
    case "Verify_LSAG": { // Verify_LSAG -> called silently by the front end to verify any LSAG signature
      const { b64Signature } = (request.params as { b64Signature: string });
      return await verifyLSAG(b64Signature);
    }

    case "Verify_SAG": { // Verify_SAG -> called silently by the front end to verify any SAG signature
      const { b64Signature } = (request.params as { b64Signature: string });
